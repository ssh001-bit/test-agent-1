...
if not GROQ_API_KEY:
    raise ValueError(
        Fore.RED + "Groq API key not found. Please set the env variable"
    )
...
def main_loop():
    ...
    if model is None:
        print(Fore.RED + "Failed to initialize model. Exiting.")
        return

    agent = Agent(model=model, result_type=Movie, system_prompt=system_prompt)
